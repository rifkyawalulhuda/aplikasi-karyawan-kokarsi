import { Controller, Get, Post, Put, Delete, Param, Body, Query, Res, UseGuards, ParseIntPipe, UseInterceptors, UploadedFile, BadRequestException, Request } from '@nestjs/common'
...
export class WarningLettersController {
  ...
  @Post()
  create(@Body() dto: CreateWarningLetterDto, @Request() req: any) {
    return this.service.create(dto, { name: req.user?.fullName ?? req.user?.name ?? 'System', role: req.user?.role ?? 'UNKNOWN' })
  }

  @Put(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateWarningLetterDto, @Request() req: any) {
