...
#   ./deploy/backup.sh --backup-dir /home/rifky/backups/kokarsi
...
if [[ "$MODE" == "docker" ]]; then
  ...
  # pg_dump langsung — stderr masuk log, stdout ke file
  docker exec kokarsi-postgres pg_dump -U kokarsi -d kokarsi_karyawan --no-password > "$DB_FILE"
  PG_EXIT=$?
else
  ...
  if [[ -f "$ENV_FILE" ]]; then
    ...
    if [[ "$DB_URL" =~ postgresql://([^:]+):([^@]+)@([^:/]+):([0-9]+)/([^"?\s]+) ]]; then
      ...
      write_log "Tidak dapat parse DATABASE_URL, menggunakan parameter default" "WARN"
  ...
  if ! command -v pg_dump &> /dev/null; then
    write_log "pg_dump not found. Install: sudo apt install postgresql-client" "ERROR"
    exit 1
  fi

  pg_dump -U "$PG_USER" -d "$PG_DB" -h "$PG_HOST" -p "$PG_PORT" -f "$DB_FILE"
  PG_EXIT=$?
fi

if [[ $PG_EXIT -ne 0 ]]; then
  write_log "pg_dump FAILED (exit code: $PG_EXIT)" "ERROR"
  exit 1
fi
...
if [[ -f "$DB_FILE" && -s "$DB_FILE" ]]; then
  ...
  write_log "Database backup FAILED: file kosong atau tidak ada" "ERROR"
  exit 1
fi
...
# .env files
for PAIR in "backend.env:${ROOT}/backend/.env" "root.env:${ROOT}/.env"; do
  DEST_NAME="${PAIR%%:*}"
  SRC="${PAIR#*:}"
  if [[ -f "$SRC" ]]; then
...
if [[ -f "$CLOUDFLARED_CONFIG" ]]; then
  ...
  write_log "Cloudflared config not found, skipping" "WARN"
fi
