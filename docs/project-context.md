...
| 181 | `triggerWindows` Email Config berlaku untuk semua modul (Kontrak Karyawan, Dokumen/Sertifikasi, Vendor, Legal): ubah cron dari window hardcoded 30 hari menjadi **multi-window loop** per `triggerWindows` dari DB. Query seleksi berbasis `expiryDate` persis N hari dari hari ini (`targetStart..targetEnd` 1 hari penuh) + dedup `hasSent(sourceType, id, window)` — bukan lagi berbasis status. EXPIRED tetap sentinel `-1`. `syncExpiredStatuses(maxDays)` (vendor & legal) terima `maxWindow` sebagai parameter. Label UI Email Config diperbarui: "Hari Pengiriman (Semua Modul)" + keterangan berlaku untuk semua modul | `backend/src/contract-cron/contract-cron.service.ts`, `backend/src/vendor-contracts/vendor-contracts.service.ts`, `backend/src/legal-koperasi/legal-koperasi.service.ts`, `app/components/settings/EmailConfigTab.vue` |
| 182 | Filter combobox multi-select di Halaman Data Karyawan: **Status Kepegawaian** (multi-select `USelectMenu` + `value-key="value"` fix), **Lokasi Kerja**, dan **Departemen** (opsi dinamis dari data yang sudah di-fetch, sorted A-Z). Tombol Reset muncul kondisional jika ada filter aktif. Semua filter reset `pageIndex=0` via watch | `app/pages/karyawan/index.vue` |
| 183 | Fix bug Tambah Karyawan: toast sukses tapi data tidak masuk. **Root cause**: Nitro proxy `server/api/employees.ts` pakai `ignoreResponseError: true` sehingga error 4xx/5xx tidak di-throw ke frontend. Fix: ganti ke `$fetch.raw` + manual status check. Backend `create()` tambah try/catch P2002 → `ConflictException(409)` dengan pesan spesifik per field (`employeeNo`/`nik`/`email`). Error highlighting field di `AddModal.vue` via tiga `ref` terpisah (`errorEmployeeNo`, `errorNik`, `errorEmail`) di-bind langsung ke prop `error` `UFormField` | `server/api/employees.ts`, `backend/src/employees/employees.service.ts`, `app/components/karyawan/AddModal.vue` |
| 184 | Fix bug Edit Karyawan: error "Internal Server Error" saat `employeeNo`/`nik`/`email` duplikat. Fix: tambah try/catch P2002 di `update()` backend → `ConflictException(409)`. `EditModal.vue` tambah tiga `ref` error + `:error` prop di `UFormField` untuk highlight field yang konflik | `backend/src/employees/employees.service.ts`, `app/components/karyawan/EditModal.vue` |
| 185 | Fix bug Import Data Karyawan: error message hanya menampilkan "Bad Request". Fix: `ImportModal.vue` tangkap `e?.data?.errors[]` dari response backend, tampilkan daftar error per baris dalam modal (badge "Baris N" + pesan spesifik, scrollable max-h-48, petunjuk duplikat) | `app/components/karyawan/ImportModal.vue` |
| 186 | Detail Drawer Surat Peringatan: klik baris tabel membuka `USlideover` dengan 6 section: **(A)** SP Escalation Timeline 3-node (SP1→SP2→SP3, node aktif berwarna), **(B)** Validity Countdown Card (progress bar `letterDate→validUntil` + angka sisa hari besar), **(C)** Jenis Pelanggaran chips berwarna per level, **(D)** Info 2-kolom (Karyawan + Pengurus), **(E)** Dokumen (Preview PDF, Unduh PDF, Unduh Dokumen Scan via `:to` prop), **(F)** Riwayat SP karyawan (fetch `?employeeId=`, klik item switch drawer). Kolom aksi tabel dihapus — semua aksi di Drawer. Fix `onSelect` prop UTable, fix nama komponen `WarningLettersDetailDrawer`, fix filter riwayat SP per karyawan (tambah `employeeId` param ke `findAll` backend + controller + Nitro proxy) | `app/components/warning-letters/DetailDrawer.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `backend/src/warning-letters/warning-letters.service.ts`, `backend/src/warning-letters/warning-letters.controller.ts`, `server/api/warning-letters/index.ts` |
| 187 | Klik kanan = Context Menu floating (via `Teleport to="body"`, `nextTick` + viewport-aware positioning) di 3 halaman: **Sertifikasi & Ijin** (Lihat Detail/Edit/Perpanjang/Unduh File/Hapus), **Surat Peringatan** (Lihat Detail/Lihat Dokumen/Unduh PDF/Edit/Unduh Scan/Hapus), **Kontrak** (Riwayat/Perpanjang/Preview/Generate/Edit/Unduh PDF/Hapus). Klik kiri = buka Detail Drawer/History Modal. Kolom Aksi tabel dihapus di semua 3 halaman. `onSelect` UTable v4 untuk klik kiri, `onContextmenu` untuk klik kanan | `app/pages/dokumen/sertifikasi-ijin/index.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/kontrak.vue` |
| 188 | Fix bug pagination UTable Nuxt UI v4: `:default-page` one-time initial value menyebabkan `UPagination` tidak reaktif saat `pageSize` berubah atau filter reset page. **Fix**: `:key="pagination-${pageSize}"` force re-mount + `:page` reactive + `watch(pageSize)` → `await nextTick(); tableApi.setPageIndex(0)` + `watch(filter)` → `tableApi.setPageIndex(0)`. Diterapkan di 6 halaman yang terdeteksi: `karyawan`, `kontrak`, `surat-peringatan`, `sertifikasi-ijin`, `settings/users`, `akte-dokumen`, `notifications`. Root cause: `v-model:pagination` UTable menggunakan getter, tapi reka-ui `PaginationRoot` (di balik `UPagination`) tidak fully-controlled — perlu `:key` untuk force re-mount saat pageSize berubah | `app/pages/karyawan/index.vue`, `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue`, `app/pages/settings/users.vue`, `app/pages/dokumen-legal/akte-dokumen/index.vue`, `app/pages/notifications/index.vue` |
| 189 | Bug fix pagination Legal Koperasi: `@update:page` hanya update `pagination.pageIndex` (Vue ref) tanpa memberitahu TanStack Table internal state. Fix: ganti ke `table?.tableApi?.setPageIndex(p - 1)` + tambah `:key="pagination-${pagination.pageSize}"` agar `UPagination` re-mount saat pageSize berubah | `app/pages/dokumen-legal/legal-koperasi/index.vue` |
| 190 | Filter "Perlu Perpanjangan / Tidak Perlu" di halaman Kontrak Customer/Vendor: tambah `needsRenewalFilter` ref, `hasActiveFilters`, `watch`, logika filter di `filteredContracts`, `USelect` di toolbar (posisi setelah filter Jenis), dan reset handler — identik dengan pola Legal Koperasi | `app/pages/dokumen-legal/kontrak-vendor/index.vue` |
| 191 | Deep-link `?openId=` dari pencarian global (`default.vue`): 4 group yang belum pakai `?openId=` (contracts, warningLetters, employeeDocuments, dokKaryawan) diperbaiki, ditambah `onMounted` + `handleOpenId()` di halaman tujuan. Kontrak & Dok. Karyawan fetch by ID dulu untuk dapat `employeeId`/`contractId` karena data server-side paginated | `app/layouts/default.vue`, `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/dok-karyawan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue` |
| 192 | Bug fix deep-link: komponen tidak re-mount saat navigasi ke route yang sama dengan `?openId=` berbeda (`onMounted` tidak dipanggil ulang). Fix: ekstrak logic ke `handleOpenId()`, panggil dari `onMounted` DAN `watch(() => route.query.openId)` di semua 7 halaman tujuan | `app/pages/kontrak.vue`, `app/pages/dokumen-legal/legal-koperasi/index.vue`, `app/pages/dokumen-legal/kontrak-vendor/index.vue`, `app/pages/dokumen-legal/akte-dokumen/index.vue`, `app/pages/dokumen/dok-karyawan/index.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue` |
| 193 | Deep-link notifikasi: `notifications.service.ts` — 8 lokasi deeplink diperbaiki ke `?openId=` pattern (4 pass harian + 4 catch-all). Migration SQL `20260820130746_fix_notification_deeplinks` dibuat + dijalankan (`npx prisma migrate deploy`) untuk update deeplink notifikasi yang sudah ada di database. Kolom PostgreSQL camelCase `"sourceId"` dengan double quotes | `backend/src/notifications/notifications.service.ts`, `backend/prisma/migrations/20260820130746_fix_notification_deeplinks/migration.sql` | Context Menu klik kanan diperluas ke 4 halaman baru: **Data Karyawan** (klik kiri = navigasi ke detail `/karyawan/:id`, klik kanan = Lihat Detail/Riwayat Kontrak/Edit Data/Offboarding/Hapus), **Kontrak Customer/Vendor** (klik kiri = buka DetailDrawer, klik kanan = Lihat Detail/Edit/Perpanjang kondisional/Unduh File kondisional/Hapus), **Legal Koperasi** (sama), **Akte Dokumen** (klik kiri = buka DetailDrawer, klik kanan = Lihat Detail/Edit/Unduh File kondisional/Hapus). Pola konsisten: `Teleport to="body"`, `data-context-menu`, `bg-default shadow-xl rounded-xl`, `z-50`, viewport-aware positioning. Kolom Aksi + `UDropdownMenu` dihapus dari semua 4 halaman | `app/pages/karyawan/index.vue`, `app/pages/dokumen-legal/kontrak-vendor/index.vue`, `app/pages/dokumen-legal/legal-koperasi/index.vue`, `app/pages/dokumen-legal/akte-dokumen/index.vue` |
| 190 | Filter Jenis Kelamin di halaman Data Karyawan: `USelect` multi-value `genderFilter` ref, item `MALE`/`FEMALE`, filter computed `filteredData`, watch + reset button, sentinel `'all'` untuk "Semua Gender" | `app/pages/karyawan/index.vue` |
| 191 | Perbaikan Export Excel (semua modul): hapus kolom `Foto` dari `toRows()`, ganti auto-width naif → manual column width per kolom dengan batas maksimum (mencegah kolom Alamat/Email terlalu lebar), tambah `ws['!freeze'] = { xSplit: 0, ySplit: 1 }` (freeze header row) di semua 6 fungsi export, nama sheet Data Karyawan menyertakan tanggal export | `app/composables/useExport.ts` |
| 192 | **Activity Log Tracker (Audit Trail)**: model `ActivityLog` di Prisma (`action`, `module`, `targetLabel`, `performedBy`, `performedByRole`, `detail`, `timestamp`, index pada `module`/`timestamp`/`performedBy`); NestJS `@Global() ActivityLogModule` (`ActivityLogService` + `ActivityLogController`); hook `void this.activityLog.log(...)` (fire-and-forget) di **10 service**: Karyawan, Kontrak, Surat Peringatan, Sertifikasi & Ijin, Kontrak Vendor, Legal Koperasi, Akte Dokumen, Master Data (8 entity), Template Kontrak, User; controller `GET /activity-logs` + `GET /activity-logs/modules` + `DELETE /activity-logs/purge` (ADMIN only); Nuxt server routes `server/api/activity-logs/`; halaman `settings/activity-log.vue` (filter Modul/Aksi/User/Date Range, tabel, pagination server-side, pengaturan retensi, purge manual); nav entry "Log Aktivitas" di sidebar Pengaturan (ADMIN only via `canManageMasterData`) | `backend/prisma/schema.prisma`, `backend/src/activity-log/`, `backend/src/app.module.ts`, `server/api/activity-logs/`, `app/pages/settings/activity-log.vue`, `app/layouts/default.vue` |
| 193 | Activity Log: **Detail kaya per operasi** — 39 `activityLog.log` calls dimodifikasi di 10 service. Contoh: Karyawan create → `Bergabung: {tgl} | Status: AKTIF`; Kontrak create → `Periode: {start} – {end} | Status: AKTIF`; SP create → `Level: SP 1 | Pelanggaran: Absensi`; User create → `Role: ADMIN | Username: john.doe`; Master Data → `Nilai: Jakarta Pusat`; Legal Koperasi + Kontrak Vendor → kondisional `needsRenewal`: tampilkan status jika perlu perpanjangan, `"Tidak perlu perpanjangan"` jika tidak — menghindari label "Tidak Aktif" yang menyesatkan untuk dokumen tanpa masa berlaku | `backend/src/employees/employees.service.ts`, `backend/src/contracts/contracts.service.ts`, `backend/src/warning-letters/warning-letters.service.ts`, `backend/src/users/users.service.ts`, `backend/src/lookups/lookups.service.ts`, `backend/src/vendor-contracts/vendor-contracts.service.ts`, `backend/src/legal-koperasi/legal-koperasi.service.ts`, `backend/src/akte-dokumen/akte-dokumen.service.ts`, `backend/src/employee-documents/employee-documents.service.ts`, `backend/src/contract-templates/contract-templates.service.ts` |
| 194 | Activity Log: **Export Excel** — `exportActivityLogsExcel()` di `useExport.ts`, 9 kolom: No/Tanggal/Jam (terpisah)/Aksi/Modul/Data/Detail/Dilakukan Oleh/Role, manual column width, freeze header, nama sheet `Log {tanggal}`; tombol Export di navbar halaman Log Aktivitas; `handleExport()` fetch ulang semua log sesuai filter aktif dengan `limit=10000` | `app/composables/useExport.ts`, `app/pages/settings/activity-log.vue`, `server/api/activity-logs/index.get.ts` |
| 195 | **Ganti semua datepicker native ke CalendarPicker UI template** di 8 form input/edit: Karyawan (AddModal/EditModal), Dok. Karyawan, Surat Peringatan (AddModal/EditModal), Sertifikasi & Ijin, Kontrak Customer/Vendor, Legal Koperasi, Akte Dokumen. Pola: `shallowRef<CalendarDate\|null>` + `watch` sync string ISO + `UPopover + CalendarPicker`. Field opsional mendapat tombol clear (`×`). `useDatePicker()` composable (`toCalDate`, `fromCalDate`, `formatDisplay`). `OffboardingModal`, `CardDetailModal` (Space), dan filter `activity-log.vue` juga dikonversi | `app/composables/useDatePicker.ts`, `app/components/CalendarPicker.vue`, 8 komponen form |
| 196 | **Upgrade filter halaman-halaman daftar** ke pola multi-select konsisten dengan Data Karyawan: **Kontrak** (Status multi + Tipe Kontrak multi, dinamis); **Surat Peringatan** (Level SP multi + Jabatan multi dinamis); **Sertifikasi & Ijin** (Status multi + Jenis Dokumen multi dinamis); **Kontrak Customer/Vendor** (alih ke client-side, Kategori/Status/Jenis multi); **Legal Koperasi** (alih ke client-side, Kategori/Status multi + Perlu Perpanjangan toggle); Semua pakai `USelectMenu multiple`, tombol Reset kondisional `hasActiveFilters`, watch reset `pageIndex=0` | `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue`, `app/pages/dokumen-legal/kontrak-vendor/index.vue`, `app/pages/dokumen-legal/legal-koperasi/index.vue` |
| 197 | **Tambah field Tanggal Mulai** di Detail Drawer Legal Koperasi dan Kontrak Customer/Vendor (kondisional `v-if="needsRenewal && startDate"`) | `app/components/legal-koperasi/DetailDrawer.vue`, `app/components/vendor-contracts/DetailDrawer.vue` |
| 198 | **Dokumentasi VitePress Activity Log** — buat `panduan-pengguna/log-aktivitas.md` (cara akses, tabel kolom, filter, export, retensi, purge, 3 skenario) dan `teknis/activity-log.md` (schema, ActivityLogModule, hook pattern, tabel 10 service); update sidebar config + `pengaturan.md` | `documentation/id/panduan-pengguna/log-aktivitas.md`, `documentation/id/teknis/activity-log.md`, `documentation/.vitepress/config.ts`, `documentation/id/panduan-pengguna/pengaturan.md` |
| 199 | **Perbaikan Backup** — `deploy/backup.ps1`: tambah Section 3 backup file konfigurasi (`.env` root/backend → `config/root.env`/`backend.env`, `~/.cloudflared/config.yml` → `config/cloudflared-config.yml`), `backup-info.json` sertakan `configCount`; `deploy/restore.ps1`: tambah Section 3 restore config dengan konfirmasi interaktif per file | `deploy/backup.ps1`, `deploy/restore.ps1` |
| 200 | **Fix bug Space Kanban card tidak bisa diklik** — root cause: `draggable="true"` wrapper di Chrome mengkonsumsi mouse events sehingga `@click` tidak fired. Fix: ganti `@click` di KanbanBoard wrapper dengan `@mousedown`/`@mouseup` + threshold 5px untuk membedakan drag vs click; diperkuat dengan fix urutan deklarasi di `CardDetailModal.vue` (`dueDateCal` dideklarasikan sebelum `cardDetail` useFetch → `ReferenceError: Cannot access 'cardDetail' before initialization`). Fix: pindahkan blok `dueDateCal` + `watch` ke setelah deklarasi `cardDetail` | `app/components/spaces/KanbanBoard.vue`, `app/components/spaces/CardDetailModal.vue` |
| 201 | **Optimasi Bug Notifikasi Bell** — `useNotifications.ts`: tambah `previousCount` ref, skip `fetchNotifications()` jika SSE broadcast count tidak berubah dari sebelumnya (fire-and-forget optimization, hanya fetch saat count benar-benar berubah) | `app/composables/useNotifications.ts` |
| 202 | **Perbaikan tampilan "masa berlaku" yang terus bertambah** — 5 komponen Detail Drawer: menampilkan "Selesai"/"Masa berlaku telah berakhir" (statis) saat masa berlaku lewat, bukan "Berakhir N hari lalu" yang terus bertambah. Progress bar di-set 0% saat expired. Kelima drawer: SP, Sertifikasi & Ijin, Kontrak Customer/Vendor, Legal Koperasi, Dok. Karyawan | `app/components/warning-letters/DetailDrawer.vue`, `app/components/sertifikasi-ijin/DetailDrawer.vue`, `app/components/vendor-contracts/DetailDrawer.vue`, `app/components/legal-koperasi/DetailDrawer.vue`, `app/components/dok-karyawan/DetailDrawer.vue` |
| 203 | **Redesign tabel Master Data** — ganti 3 daftar `<ul>` dengan `UTable` dinamis per tab, kolom per resource (lookup: No/Nama/Aksi; dokumen: No/Nama/Tipe/Penerbit/Kategori/Aksi; perusahaan: No/Nama/Kontak/Alamat/Aksi), sortableHeader, pagination (15/30/50/100), tombol Aksi selalu terlihat (tidak hover-only), UCard wrapper dengan header, search toolbar, inline add form, fix badge color dokumen (blue/green → info/success/warning/error), tab horizontal scroll mobile. Fix bug template double-tag: duplicate modal block di akhir file dihapus | `app/pages/settings/master-data.vue` |
| 204 | **Fix pagination Log Aktivitas** — halaman tidak update saat tekan halaman 2. Root cause: `getPaginationRowModel()` melakukan client-side slicing pada data yang sudah di-slice server-side. Fix: hapus `v-model:pagination`, `getPaginationRowModel`, dan `:pagination-options` dari `UTable` — pagination sepenuhnya server-side via `fetchLogs()` | `app/pages/settings/activity-log.vue` |
| 205 | **Perbaikan 70 TypeScript error** — fix semua typecheck error di seluruh project: (A) UModal `:ui` slot v2→v4 — `width`/`container`/`panel` → `content`/`wrapper`; (B) `route.query` null — perlebar signature `handleOpenId`; (C) modul spaces — null guard, fallback index, Tiptap v3 API `setContent({emitUpdate:false})`, `handlePaste` sync; (D) sertifikasi-ijin/FormModal null guard; (E) kalender `window` di template → computed `tooltipStyle`; (F) kontrak UButton resolve + statusColorMap; (G) settings profileInitials optional chaining; (H) nuxt.config `@ts-expect-error`; (I) badge color semantik, useFetch generic, notes type. Tambah route proxy baru `server/api/spaces/[id]/members/[userId].delete.ts` (fix bug runtime hapus member Space) | semua file relevan |
| 206 | **Batasan Hapus untuk role Pengelola Koperasi** — Data Karyawan dan Kontrak: (1) backend guard `ForbiddenException` di `@Delete` endpoint; (2) `canDelete = computed(() => role === 'ADMIN')` di auth store; (3) context menu "Hapus" di-hide via `v-if="auth.canDelete"` + divider kondisional di halaman Karyawan dan Kontrak | `backend/src/employees/employees.controller.ts`, `backend/src/contracts/contracts.controller.ts`, `app/stores/auth.ts`, `app/pages/karyawan/index.vue`, `app/pages/kontrak.vue` |
| 207 | **Bug notifikasi muncul lagi setelah refresh** — 3 bug di backend: (1) cron `generateNotifications` catch-all me-reset `isRead:false, readAt:null` → dihapus; (2) `markOneRead` tidak broadcast SSE → tambah `broadcastUnreadCount()`; (3) `markAllRead` tidak broadcast SSE → tambah `broadcastUnreadCount()`; (4) `markOneRead` `where` variable tidak dipakai → tambah `findFirst` ownership check. Sorting `findAll` diubah ke `isRead asc + createdAt desc` (unread first, terbaru di atas) | `backend/src/notifications/notifications.service.ts` |
| 208 | **Notifikasi real-time saat karyawan ditambahkan ke agenda Kalender** — method baru `generateAgendaCreatedNotifications` di `NotificationsService`; dipanggil dari `CalendarService.create()` dan `CalendarService.update()` (diff `assignedUserIds` lama vs baru — kirim notif hanya ke member baru); deeplink `/kalender?openId={agendaId}`; halaman Kalender tambah `handleOpenId` + `onMounted` + `watch` untuk membuka form edit agenda dari query param | `backend/src/notifications/notifications.service.ts`, `backend/src/calendar/calendar.service.ts`, `backend/src/calendar/calendar.controller.ts`, `app/pages/kalender/index.vue` |
| 209 | **Notifikasi saat ditambahkan sebagai Member Space** — method baru `notifyMemberAdded` di `SpaceNotificationService`; `SpacesService.addMember()` di-inject `SpaceNotificationService` + tambah param `actorName` + panggil notif; `SpacesController.addMember()` teruskan `actorName` dari JWT; broadcast SSE real-time via `broadcastUnreadToClients()` | `backend/src/spaces/space-notification.service.ts`, `backend/src/spaces/spaces.service.ts`, `backend/src/spaces/spaces.controller.ts` |
| 210 | **Tambah Member saat Buat Space baru** — form Create Space tambah field "Anggota" (`USelectMenu` multi-select, opsional); fetch `/api/users/pengurus`; `form.memberIds[]` ikut terkirim di body POST (backend `CreateSpaceDto.memberIds` sudah ada); `SpacesService.create()` kirim `notifyMemberAdded` fire-and-forget ke setiap member yang dipilih | `app/components/spaces/CreateSpaceModal.vue`, `backend/src/spaces/spaces.service.ts` |
| 211 | **Fix bug Space: hanya creator yang bisa Hapus Space** — tombol "Hapus Space" di dropdown tersembunyi untuk non-creator. Computed `isOwner = space.createdById === auth.admin?.id`; `menuItems` filter opsi "Hapus Space" berdasarkan `isOwner`. Backend sudah punya guard `checkOwner()` (fix UX saja) | `app/pages/spaces/[id].vue` |
| 212 | **Link "Lihat Semua" di Detail Karyawan dengan filter nama** — tambah prop `employeeName` ke 3 komponen (ContractTimeline, WarningLetterList, EmployeeDocumentList); link menuju `/kontrak?search={nama}`, `/dokumen/surat-peringatan?search={nama}`, `/dokumen/sertifikasi-ijin?search={nama}`; 3 halaman target membaca `route.query.search` di `onMounted` dan set `searchQuery` | `app/components/karyawan/detail/ContractTimeline.vue`, `app/components/karyawan/detail/WarningLetterList.vue`, `app/components/karyawan/detail/EmployeeDocumentList.vue`, `app/pages/karyawan/[id].vue`, `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue` |
| 213 | **Filter Status Dok. Karyawan** — tambah `statusFilter` ref (sentinel `'all'`), `USelect` di toolbar (kanan), teruskan ke `useFetch` query (`status: statusFilter !== 'all' ? ... : undefined`), watch reset `page=1`, `hasActiveFilters`, `resetFilters()`. Backend: controller `findSummary` tambah `@Query('status')` + teruskan ke `findEmployeeSummary`; service tambah param `status` + filter `employeeDocuments: { some: { status } }`; Nuxt proxy `summary.get.ts` teruskan `status` param | `app/pages/dokumen/dok-karyawan/index.vue`, `backend/src/employee-documents/employee-documents.controller.ts`, `backend/src/employee-documents/employee-documents.service.ts`, `server/api/employee-documents/summary.get.ts` |
| 214 | **Redesign tabel Dok. Karyawan — inline chips per dokumen** — ganti kolom "Jumlah Dokumen" + "Status" dengan kolom "Kelengkapan Dokumen": 9 slot PERSONAL fix (KTP/SIM/NPWP/KK/PASPOR/BPJS_TK/BPJS_KES/IJAZAH/SERTIFIKAT) berwarna sesuai status atau outline dashed abu jika belum diinput; chip CERTIFICATION max 3 + overflow `+N lainnya`; `UTooltip` hover tiap chip (nama · nomor · tanggal expiry). Real-time update: tambah `refresh: refreshSummary` dari `useFetch` + `@saved="refreshSummary"` ke `DokKaryawanDetailDrawer` | `app/pages/dokumen/dok-karyawan/index.vue` |
| 215 | **Fix bug ganti password — tidak validasi & tidak benar-benar berubah** — root cause: `security.vue:onSave()` hanya cek konfirmasi password, langsung toast sukses tanpa API call; `server/api/users.ts` pakai `ignoreResponseError: true` menelan error 400. Fix: (1) buat Nuxt proxy `server/api/auth/change-password.put.ts` teruskan ke backend `PUT /auth/change-password`; (2) `security.vue` ubah `onSave()` menjadi async dengan `$fetch` + error handling + loading state | `app/pages/settings/security.vue`, `server/api/auth/change-password.put.ts` |
| 216 | **Redesign halaman Keamanan** — 2 kartu modern: (1) Ubah Password: divider visual Password Saat Ini vs Baru, show/hide toggle per field, strength indicator (Lemah/Sedang/Kuat/Sangat Kuat), inline match indicator real-time; (2) Info Akun: nama, no. karyawan, role badge, tipe akun badge, hint ke Profil Akun. Layout `max-w-2xl p-4 sm:p-6` konsisten settings lain | `app/pages/settings/security.vue` |
| 217 | **Log aktivitas ganti password** — inject `ActivityLogService` ke `AuthController`; setelah `changePassword`/`changeUserPassword` berhasil, catat `{ action: 'UPDATE', module: 'Keamanan', targetLabel: 'Ubah Password — {nama}' }` fire-and-forget | `backend/src/auth/auth.controller.ts` |
| 218 | **Fix pagination bug Master Data & Users** — pola sama: watcher filter/search hanya mutasi `pagination.pageIndex = 0` langsung tanpa sync `tableApi`; `@update:page` hanya update `tableApi` tanpa update `pagination.pageIndex`; `:key` UPagination tidak include `pageIndex`. Fix: tambah `await nextTick(); table.value?.tableApi?.setPageIndex(0)` di watcher, tambah `pagination.pageIndex = p - 1` di handler, `:key="pagination-${pageSize}-${pageIndex}"` | `app/pages/settings/master-data.vue`, `app/pages/settings/users.vue` |
| 219 | **Redesign halaman Template Kontrak** — grouped by family (PKWT/MITRA) dengan section header ikon+ring berwarna, kartu `border-l-4` warna per family/status, metadata grid 2-kolom, badge "Dikustomisasi", footer: tombol "Edit Konten" (primary full-width) + Edit + Hapus; grid 3-kolom (`lg:grid-cols-3`), empty state per family, info bar modern, `templatesByFamily` computed, `familyConfig` constant | `app/pages/settings/contract-templates.vue` |
| 220 | **Fix bug hapus karyawan FK violation** — `employees.service.ts:remove()` tidak menghapus `masterAdmin` sebelum hapus employee. Fix: tambah `employeeNo` ke select, tambah `tx.masterAdmin.deleteMany({ where: { employeeNo } })` di dalam transaksi sebelum `tx.employee.delete` | `backend/src/employees/employees.service.ts` |
| 221 | **Fix bug buat user — validasi email tidak aktif & toast sukses palsu** — root cause: (1) `server/api/users.ts` pakai `ignoreResponseError: true` sehingga 400 dari backend tidak di-throw; (2) `users.vue:saveUser()` tidak validasi format email di frontend. Fix: ganti proxy ke `$fetch.raw` + manual status check; tambah regex email di `saveUser()` sebelum kirim | `server/api/users.ts`, `app/pages/settings/users.vue` |
| 222 | **Deploy mesin baru — perbaikan kritis** — (1) `prisma.config.ts`: tambah `dotenv` eksplisit load `.env` agar `DATABASE_URL` terbaca (tidak fallback ke port hardcoded `5435`); (2) `seed.ts`: hapus field `positionLabel`/`workLocationLabel` yang sudah tidak ada di schema `Contract`; (3) `ecosystem.config.cjs`: ganti semua path hardcoded `E:/Github/...` ke `__dirname`-based dynamic paths (`ROOT`, `BACKEND`, `LOGS`); (4) `deploy/start.ps1` mode native: baca port dari `DATABASE_URL` di `.env` + fix PowerShell 5.x `?.Line` syntax | `backend/prisma.config.ts`, `backend/prisma/seed.ts`, `ecosystem.config.cjs`, `deploy/start.ps1` |
| 223 | **Deploy mesin baru — perbaikan backup/restore native** — `backup.ps1` & `restore.ps1`: (1) hapus `ignoreResponseError`/`--no-password`, (2) baca `DATABASE_URL` dari `backend/.env` parse user/password/host/port/db via regex, (3) gunakan variabel `$pgUserFinal`/`$pgPortFinal` (bukan parameter script) agar scoping PowerShell benar, (4) fallback ke `kokarsi2026` jika parse gagal; `setup-backup-schedule.ps1`: tambah param `-Mode`, teruskan ke `backup.ps1`; file baru `SETUP-BACKUP-SCHEDULE-NATIVE.bat` + cek administrator | `deploy/backup.ps1`, `deploy/restore.ps1`, `deploy/setup-backup-schedule.ps1`, `deploy/SETUP-BACKUP-SCHEDULE-NATIVE.bat` |
| 224 | **Panduan deploy mesin baru** — `docs/panduan-deploy.md`: tambah section "Deploy Database Prisma & Seed (Mesin Baru)" dengan urutan `npm install → prisma generate → prisma migrate deploy → npx prisma db push (jika ada kolom hilang) → seed`; catatan `prisma.config.ts` + `dotenv`; troubleshooting tabel error P1001/P3009/P2022; update section backup/restore native dengan info port dari `.env`; dokumentasi bug `positionLabel`/`workLocationLabel` seed | `docs/panduan-deploy.md` |
| 202 | **Bug fix pagination Legal Koperasi**: `@update:page` hanya update `pagination.pageIndex` (Vue ref) tanpa memberitahu TanStack Table internal state. Fix: ganti ke `table?.tableApi?.setPageIndex(p - 1)` + tambah `:key="pagination-${pagination.pageSize}"` agar `UPagination` re-mount saat pageSize berubah | `app/pages/dokumen-legal/legal-koperasi/index.vue` |
| 203 | **Filter "Perlu Perpanjangan / Tidak Perlu" di halaman Kontrak Customer/Vendor**: tambah `needsRenewalFilter` ref, `hasActiveFilters`, `watch`, logika filter di `filteredContracts`, `USelect` di toolbar, dan reset handler — identik dengan pola Legal Koperasi | `app/pages/dokumen-legal/kontrak-vendor/index.vue` |
| 204 | **Deep-link `?openId=` dari pencarian global** (`default.vue`): 4 group yang belum pakai `?openId=` (contracts, warningLetters, employeeDocuments, dokKaryawan) diperbaiki + `onMounted` + `handleOpenId()` di halaman tujuan. Bug fix deep-link: komponen tidak re-mount saat navigasi ke route sama dengan `?openId=` berbeda — ekstrak logic ke `handleOpenId()` + `watch(() => route.query.openId)` di semua 7 halaman | `app/layouts/default.vue`, `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/dok-karyawan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue`, `app/pages/dokumen-legal/akte-dokumen/index.vue` |
| 205 | **Deeplink notifikasi diperbaiki**: `notifications.service.ts` — 8 lokasi deeplink diganti ke `?openId=` pattern (4 pass harian + 4 catch-all). Migration SQL `20260820130746_fix_notification_deeplinks` (PostgreSQL, kolom camelCase `"sourceId"` dengan double quotes) dibuat + dijalankan untuk update notifikasi yang sudah ada di DB | `backend/src/notifications/notifications.service.ts`, `backend/prisma/migrations/20260820130746_fix_notification_deeplinks/migration.sql` |
| 206 | **Hapus field `positionLabel` & `workLocationLabel`** dari form Kontrak Karyawan (Add/Edit/Renew), DTO, Prisma payload, dan schema — field tidak pernah dirender ke PDF (fallback selalu ke `employee.jobRole?.name`/`workLocation?.name`). Migration `20260820191341_drop_positionLabel_workLocationLabel` dibuat + dijalankan | `app/components/kontrak/AddContractModal.vue`, `app/components/kontrak/EditContractModal.vue`, `app/components/kontrak/RenewContractModal.vue`, `backend/src/contracts/contracts.service.ts`, `backend/src/contracts/contract-document.service.ts`, `backend/prisma/schema.prisma` |
| 207 | **PKWT PDF row-aligned columns**: `renderParallelColumns()` ditulis ulang dari independent-flow ke paired-flow — setiap pasangan blok ID\|EN dirender pada posisi Y yang sama, advance `max(tinggiID, tinggiEN)`; kolom lebih pendek boleh kosong. Closing paragraph ("Demikian Kesepakatan...") dipindahkan ke dalam kotak kolom sebagai blok terakhir setelah Pasal 11 (sebelumnya di luar garis). Helper `computeBlockHeight()` baru | `backend/src/contracts/contract-document.service.ts` |
| 208 | **Dashboard improvements**: section "Perlu Perhatian" (6 kartu: Kontrak Akan Habis, Kontrak Vendor, Legal Koperasi, Sertifikasi, SP Aktif, Notifikasi) dengan item klik-able `?openId=`; 2 KPI cards tambahan (Sertifikasi Akan Expired, SP Aktif); Today Snapshot (agenda hari ini di header); badge total attention count. Backend `getDashboardStats()` tambah query expiring (contract/vendor/legal/cert items + activeWarnings) + `dashboardCache.invalidate()` di 3 service (vendor, legal, dokumen). Komponen `DashboardAttentionCard.vue` (nama diprefix `Dashboard-` karena folder) | `app/pages/index.vue`, `app/components/dashboard/AttentionCard.vue`, `backend/src/employees/employees.service.ts`, `backend/src/vendor-contracts/vendor-contracts.service.ts`, `backend/src/legal-koperasi/legal-koperasi.service.ts`, `backend/src/employee-documents/employee-documents.service.ts` |
| 209 | **Tombol "Perpanjang" di DetailDrawer Kontrak Vendor & Legal Koperasi**: tambah emit `renew` + tombol di footer (kondisional `needsRenewal && (AKAN_BERAKHIR || EXPIRED) && !renewedTo`); handler `@renew` di index.vue menutup drawer lalu buka FormModal mode renew. Backend guard status di `renewContract()`/`renewLegal()` (tolak selain AKAN_BERAKHIR/EXPIRED via `BadRequestException`). Datepicker `minDate` — disable tanggal sebelum `endDate` kontrak lama saat mode renew (`minStartDateCal`); validasi tanggal di `onSubmit` (startDate >= endDate kontrak lama, toast error) | `app/components/vendor-contracts/DetailDrawer.vue`, `app/components/vendor-contracts/FormModal.vue`, `app/components/legal-koperasi/DetailDrawer.vue`, `app/components/legal-koperasi/FormModal.vue`, `app/pages/dokumen-legal/kontrak-vendor/index.vue`, `app/pages/dokumen-legal/legal-koperasi/index.vue`, `backend/src/vendor-contracts/vendor-contracts.service.ts`, `backend/src/legal-koperasi/legal-koperasi.service.ts` |
| 210 | **Preview foto profil karyawan**: di halaman Data Karyawan & Detail Karyawan, klik avatar foto membuka `UModal` preview (header nama karyawan + foto full `max-h-[70vh]`); `e.stopPropagation()` agar tidak trigger row select; fallback inisial tetap untuk yang tidak punya foto | `app/pages/karyawan/index.vue`, `app/components/karyawan/detail/ProfileHeader.vue` |
| 211 | **Fix background transparan**: context menu floating di 7 halaman — `bg-default` → `bg-white dark:bg-neutral-900` (solid); tooltip chart di Dashboard (6 tooltip) — `bg-background` → `bg-white dark:bg-neutral-900` | `app/pages/karyawan/index.vue`, `app/pages/kontrak.vue`, `app/pages/dokumen/surat-peringatan/index.vue`, `app/pages/dokumen/sertifikasi-ijin/index.vue`, `app/pages/dokumen-legal/legal-koperasi/index.vue`, `app/pages/dokumen-legal/kontrak-vendor/index.vue`, `app/pages/dokumen-legal/akte-dokumen/index.vue`, `app/pages/index.vue` |
| 212 | **Calendar month/year quick-jump picker**: header label Month View jadi clickable `UPopover` (navigasi tahun ← →, grid 4×3 bulan, bulan aktif `bg-primary`, bulan sekarang ring tipis); `jumpToMonth()` + `openPicker()` | `app/pages/kalender/index.vue` |
| 213 | **Space Dokumen download PDF**: composable `useExportDocPdf.ts` (parse ProseMirror JSON Tiptap → PDF via jspdf, dynamic import, sanitasi karakter non-Latin-1 untuk font Helvetica, heading/paragraf/list/blockquote/codeBlock, footer halaman); tombol "Download PDF" di modal editor (`SpaceDocsView.vue`) & full-page (`[id]-docs-[docId].vue`), simpan dulu jika dirty sebelum generate | `app/composables/useExportDocPdf.ts`, `app/components/spaces/SpaceDocsView.vue`, `app/pages/spaces/[id]-docs-[docId].vue` |
| 225 | **Fix workspace pnpm** — hapus `confirmModulesPurge` dari `pnpm-workspace.yaml` (setting pnpm v11, tidak dikenal pnpm 12.5.1 sehingga `pnpm dev` gagal dengan `ERR_PNPM_UNRECOGNIZED_WORKSPACE_SETTINGS`) | `pnpm-workspace.yaml` |
| 226 | **Modul Pemakaian Kendaraan** (semua role, akses penuh): Prisma `OperationalVehicleUsage` (usedAt, vehicleNumber hardcoded `Xenia B 2845 FON`/`Grand max B 9043 FCM`, driver/destination/user/requester, status nullable hanya `BATAL` + audit cancel `cancelledAt/ByName/ByRole`, createdByName/Role); backend module (GET list, POST create, POST `:id/cancel` atomik via `updateMany where status:null` — **tanpa endpoint edit/hapus**, immutable); Nitro proxy; halaman `/operasional/pemakaian-kendaraan` (menu antara Kalender & Space, label "Pemakaian Kendaraan"); form Add (CalendarPicker + input jam → `usedAt` `+07:00` WIB, format tampil `DD/MM/YYYY HH:mm`); konfirmasi `confirmActionToast` saat tekan Batal, tombol hilang setelah Batal + tampilkan info pembatal (nama·role·waktu); badge label `Terjadwal`/`Batal`; activity log CREATE/UPDATE | `backend/prisma/schema.prisma`, `backend/prisma/migrations/20260922000000_add_operational_vehicle_usage/`, `backend/src/operational-vehicle-usage/`, `server/api/operational-vehicle-usages/`, `app/pages/operasional/pemakaian-kendaraan.vue`, `app/layouts/default.vue` |
| 227 | **Struktur & filter tabel Pemakaian Kendaraan konsisten modul lain** — UTable ui identik Karyawan/Kontrak Vendor (rounded th, border, hover row), page size 15/30/50/100, empty state icon car; toolbar: search (driver/tujuan/user/requester/plat), filter Status (Semua/Terjadwal/Batal), **filter rentang tanggal** (dua `CalendarPicker` "Dari/Sampai" di popover + tombol Terapkan, inclusive, perhitungan tanggal WIB via `Intl.formatToParts`, tombol X + Reset kondisional); **aturan urutan default hybrid**: semua data hari ini di atas diurut jam ascending, data sebelumnya tanggal terbaru→terlama (WIB), implementasi di `sortedUsages` saat `sort.key==='usedAt' && direction==='asc'`; backend fallback `orderBy usedAt desc/id desc`; sorting manual via `sortableHeader` tetap tersedia | `app/pages/operasional/pemakaian-kendaraan.vue`, `backend/src/operational-vehicle-usage/operational-vehicle-usage.service.ts` |
| 228 | **Export Excel Pemakaian Kendaraan + seed mock 100 data** — tombol Export membuka modal pilihan `Semua`/`Bulan` (select bulan+tahun, pola modul lain), filter export per WIB, kolom audit pembatal (Waktu/Dibatalkan Oleh/Role), nama file `{slug}-semua` / `{slug}-{tahun}-{bulan}`; label status UI "Belum Batal" → **"Terjadwal"** (DB tetap null); seed `backend/prisma/seed.ts` idempotent **100 record mock** (deleteMany prefix `[MOCK]` lalu createMany deterministik, ~11% BATAL, rentang 100 hari terakhir) untuk uji tabel/filter/pagination/export | `app/composables/useExport.ts`, `app/pages/operasional/pemakaian-kendaraan.vue`, `backend/prisma/seed.ts` |
| 229 | **Section Dashboard "Pemakaian Kendaraan"** — backend `getDashboardStats` tambah `vehicleUsage.{today,nextSevenDays}` (boundary `startOfTodayWib`/`endOfSevenDaysWib` via offset UTC+7, count total/active/cancelled + items ascending + `vehicleCounts` groupBy vehicleNumber); `dashboardCache.invalidate()` dipanggil saat create/cancel pemakaian; frontend section di antara "Perlu Perhatian" & "Distribusi" (collapse state localStorage `dashboard-section-kendaraan`, ikut Expand/Collapse All): 3 KPI card (Pemakaian/Terjadwal/Batal), card jadwal (baris jam+plat+driver+tujuan+badge), card distribusi per kendaraan (bar relatif terhadap max), badge header `{n} hari ini`, jarak `mt-4` antar grid | `backend/src/employees/employees.service.ts`, `backend/src/operational-vehicle-usage/operational-vehicle-usage.service.ts`, `app/pages/index.vue` |
| 230 | **Dashboard tab `Hari Ini` / `7 Hari ke Depan`** — segmented control role=tab default `Hari Ini`; periode 7 hari = hari ini + 6 hari berikutnya (semua data, tanpa limit 5); 3 KPI + judul card + badge header mengikuti tab; jadwal mingguan **dikelompokkan per tanggal** (header sticky blur: "Senin, 22 September" + count + badge `Hari Ini`), area scroll `max-h-96 overflow-y-auto`, empty state per periode; helper `jamWib/tanggalWib/dateKeyWib` semua `Asia/Jakarta` | `app/pages/index.vue` |
| 231 | **Fix logout tidak mengakhiri sesi + 401 SSR** — root cause: `auth_token` dibuat server sbg cookie `httpOnly` tapi `auth.logout()` hanya mengosongkan state client → refresh auto-login lagi; fix: endpoint `POST /api/auth/logout` (`deleteCookie` `auth_token`+`auth_admin`), `auth.logout()` menjadi async (fetch endpoint → clear state → navigateTo `/login`, tetap clear walau fetch gagal); halaman Pemakaian Kendaraan fetch data hanya `onMounted` jika `auth.isLoggedIn` (hilangkan 401 unhandled saat SSR/refresh) + auto `auth.logout()` saat API membalas 401 | `server/api/auth/logout.post.ts`, `app/stores/auth.ts`, `app/pages/operasional/pemakaian-kendaraan.vue` |
| 232 | **Modul Arsip Umum** (submenu Dokumen Legal tepat di bawah Akte Dokumen, route `/dokumen-legal/arsip-umum`): Prisma `GeneralArchive` — documentName wajib, documentNumber/createdDate/expiryDate/notes/fileUrl **opsional** + enum `NotificationCategory.AR SIP_UMUM` + migration `20260922010000_add_general_archives` (jalankan `prisma migrate deploy`, error 500 jika belum); backend `general-archives` module: GET list (pagination+search), GET/POST/PUT/DELETE `:id` + activity log, `POST :id/file` diskStorage `uploads/general-archives` (PDF/JPG/JPEG/PNG/WEBP, 10MB, magic bytes, file lama dihapus saat ganti/hapus); Nitro proxy 3 file; **notifikasi expiry opsional**: pass TRIGGER_DAYS `[90,60,30,7,0]` di `generateNotifications` — hanya record dgn `expiryDate` terisi, kategori `ARSIP_UMUM`, deeplink `/dokumen-legal/arsip-umum?openId={id}`; **scheduler**: `orphaned-files-cleanup` tambah `generalArchive.findMany({fileUrl})` → `addPath` agar file tidak dihapus jam 02:00 WIB; kolom tabel: Dokumen, Nomor, Tanggal Dibuat, Tanggal Berakhir, Keterangan, File | `backend/prisma/schema.prisma`, `backend/prisma/migrations/20260922010000_add_general_archives/`, `backend/src/general-archives/`, `backend/src/notifications/notifications.service.ts`, `backend/src/contract-cron/contract-cron.service.ts`, `server/api/general-archives/`, `app/pages/dokumen-legal/arsip-umum/index.vue`, `app/layouts/default.vue` |
| 233 | **Arsip Umum: form upload UX + Detail Drawer modern** — FormModal upload mengikuti pola Dokumen Legal: panel "File dokumen tersimpan" (icon paperclip + Unduh + tombol Ganti), chip file baru (icon file-check + nama + ukuran MB + tombol X batalkan), tombol dropzone-style ("Pilih file (pengganti)" bawa native input tersembunyi), description format+ukuran; **DetailDrawer redesign**: header (icon archive + label modul + nama + nomor mono), badge status masa berlaku (`Masih berlaku`/`Sudah berakhir`/`Tanpa batas waktu`), 2 kartu tanggal, block keterangan `min-h`, panel File Dokumen (icon PDF/gambar + Unduh icon + area preview toggle `PdfViewer`/img + tombol "Lihat Preview Dokumen"/"Sembunyikan Preview"), empty state "Belum ada file dokumen", tombol Edit sticky bottom | `app/components/arsip-umum/FormModal.vue`, `app/components/arsip-umum/DetailDrawer.vue` |
| 234 | **Arsip Umum: interaksi tabel + export per tahun** — **kolom Aksi dihapus**; klik kiri baris = buka Detail Drawer (`on-select` UTable) dgn hover highlight; klik kanan = context menu floating (`Teleport`, viewport-aware) Lihat Detail / Edit / Lihat Preview + Unduh File (kondisional file) / Hapus; **kolom File = tombol Preview** → popup modal `sm:max-w-4xl` (PDF via `PdfViewer` h-70vh, gambar `object-contain` max-h-70vh) bukan buka tab; tombol Export membuka **modal Tahun Export** (opsi `Semua Tahun` + tahun dari `createdDate`, record tanpa `createdDate` hanya ikut Semua Tahun, helper text, nama file `arsip-umum-semua`/`arsip-umum-{tahun}`, toast warning jika kosong) | `app/pages/dokumen-legal/arsip-umum/index.vue`, `app/composables/useExport.ts` |
## Arsitektur
